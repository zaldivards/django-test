...
def validate_max_date(previous_date: date, new_date: date):
    if new_date < previous_date:
        raise ValidationError(
                "The minimum date to return a movie it's today or the previously set date"
            )
...
class MovieFrom(ModelForm):
    ...
    def clean(self):
        ...
        if self.request == 'PUT':
            existent_rentals = Rentals.objects.get_rented_copies(self.instance.movie_id)['rented_copies']
            existent_rentals = existent_rentals if existent_rentals is not None else 0
